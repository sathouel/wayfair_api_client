...
class WayfairAPICLient:
    ...
    def endpoints(self):
        endpoints = {
            'api': self.SANDBOX_BASE_URL if self._sandbox else self.BASE_URL,
            'gql': utils.urljoin(self.BASE_URL, 'v1/graphql'),
        }
        return endpoints
    ...
    def _authtenticate(self):
        ...
        token_type, access_token = res.json().get('token_type'), res.json().get('access_token')
        auth_headers = {
